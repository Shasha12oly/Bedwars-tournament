...
import { getTournamentsFromDatabase, saveTeamToDatabase, getTeamsFromDatabase, getTeamCountFromDatabase } from '@/lib/tournament-storage';
...
export default function TournamentRegister({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const loadTournament = async () => {
      try {
        // Get tournament and teams data from server API
        ...
        if (tournamentData) {
          // Calculate registered teams for this tournament
          const registeredTeams = teams.filter((team: any) => team.tournamentId === resolvedParams.id);
          const tournamentWithCount = {
            ...tournamentData,
            registered: registeredTeams.length,
            slots: tournamentData.max_slots,
            // Parse JSON fields from database
            rules: typeof tournamentData.rules === 'string' ? JSON.parse(tournamentData.rules) : tournamentData.rules,
            prize: tournamentData.prize_pool // Map prize_pool to prize for display
          };
          
          console.log('Registration page - Tournament:', tournamentWithCount);
          console.log('Registration page - Registered teams:', registeredTeams.length);
          
          setTournament(tournamentWithCount);
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Success - handle auto-closure and bracket generation message
      if (result.registrationAutoClosed && result.bracketAutoGenerated) {
        // Show special message for tournament full with bracket generated
        router.push(`/tournaments/${resolvedParams.id}?registered=true&autoClosed=true&bracketGenerated=true&refresh=${Date.now()}`);
      } else if (result.registrationAutoClosed) {
        // Show special message for tournament full
        router.push(`/tournaments/${resolvedParams.id}?registered=true&autoClosed=true&refresh=${Date.now()}`);
      } else {
        // Normal registration success
        router.push(`/tournaments/${resolvedParams.id}?registered=true&refresh=${Date.now()}`);
      }
  ...
  const validateForm = () => {
    ...
    if (tournament.format === 'Rankedbedwars 4v4') {
  ...
  const getTeammateFields = () => {
    ...
    // For both Duo and 4v4 formats, show team name and teammates
    const teammateCount = tournament.format === 'Duo' ? 1 : (tournament.format === 'Rankedbedwars 4v4' ? 3 : 3);
          ...
          <div key={index} className="space-y-2">
            <div>
              ...
              <input
                ...
                placeholder="Enter teammate's Minecraft IGN"
                required
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-1">
                Teammate {index + 1} Discord
              </label>
              <input
                type="text"
                ...
                className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-emerald-400"
                placeholder="Enter teammate's Discord username"
                required
              />
            </div>
          </div>
  ...
  // Check if tournament is full or registration closed
  if (tournament && (tournament.registered >= tournament.slots || (tournament.status !== 'open' && tournament.status !== 'upcoming'))) {
    const reason = tournament.registered >= tournament.slots ? 'Tournament Full' : 'Registration Closed';
    const message = tournament.registered >= tournament.slots 
      ? 'This tournament has reached maximum capacity.' 
      : tournament.status === 'in_progress' ? 'The tournament has started and registration is closed.' 
      : tournament.status === 'completed' ? 'The tournament has completed.' 
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center">
          <div className="text-center max-w-md mx-auto px-4">
            ...
            <h1 className="text-3xl font-bold text-white mb-4">{reason}</h1>
            <p className="text-slate-400 mb-6">
              {message}
            </p>
            <p className="text-slate-400 mb-8">
              {tournament.status === 'completed' && 'Check the winners tab to see who won!'}
              {tournament.status === 'in_progress' && 'Matches are currently in progress.'}
              {tournament.status === 'closed' && 'The tournament has concluded.'}
              {tournament.registered >= tournament.slots && `Registered: ${tournament.registered}/${tournament.slots} teams`}
            </p>
            <Link 
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="mt-2 space-y-1">
                  {tournament.prize ? tournament.prize.split('\n').map((prize: string, index: number) => (
